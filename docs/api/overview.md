...
# Backend Documentation (Continued)
...
| Test Type         | Metrics Tracked                       | Threshold                 |
| ----------------- | ------------------------------------- | ------------------------- |
| Load Testing      | Response time, throughput, error rate | < 500ms p95               |
| Stress Testing    | Breaking point, recovery time         | Handles 2x peak load      |
| Endurance Testing | Memory leaks, degradation over time   | Stable over 8 hours       |
| Spike Testing     | Response under sudden load spikes     | Recovers within 2 minutes |
...
| --------------------- | ----------------------------------- |
| Authentication Bypass | Verify JWT cannot be forged         |
| SQL Injection         | Verify ORM prevents injection       |
| XSS Prevention        | Verify input sanitization           |
| Rate Limiting         | Verify limits are enforced          |
| IDOR Prevention       | Verify data isolation between users |
| Session Security      | Verify token expiry and revocation  |
